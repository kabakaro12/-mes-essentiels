...
function updateAccountButton(){const b=document.querySelector('#accountBtn');if(!b)return;b.textContent='👤';b.title=currentUser?`Mon compte — ${currentUser.name}`:'Mon compte';b.setAttribute('aria-label',b.title)}
function showAccount(message=''){if(currentUser){const id=currentUser.phone?`📱 +${esc(currentUser.phone)}`:(currentUser.email?`✉️ ${esc(currentUser.email)}`:'');modal(`<p class="eyebrow dark">MON COMPTE</p><h2>Bonjour ${esc(currentUser.name)}</h2><p>${id}</p>${currentUser.email?`<p>${esc(currentUser.email)}</p>`:''}<button id="changePwdBtn" class="btn ghost dark-text full">Modifier mon mot de passe</button><button id="logoutBtn" class="btn gold full">Se déconnecter</button>`);document.querySelector('#changePwdBtn').onclick=showChangePassword;document.querySelector('#logoutBtn').onclick=async()=>{try{await api('/logout',{method:'POST'})}catch{} token='';currentUser=null;localStorage.removeItem('mesEssentielsToken');closeModal();updateAccountButton()};return}modal(`<p class="eyebrow dark">MON COMPTE</p><h2>Connexion</h2>${message?`<p class="form-msg" style="color:#8a6500">${esc(String(message))}</p>`:''}<form id="loginForm" class="modal-form"><input name="login" placeholder="Téléphone ou e-mail" required><input name="password" type="password" placeholder="Mot de passe" required><button class="btn gold">Se connecter</button></form><button id="forgotPassword" class="text-button" type="button">Mot de passe oublié ?</button><div class="modal-sep">ou</div><h3>Créer un compte</h3><form id="registerForm" class="modal-form"><input name="name" placeholder="Nom complet" required><input name="phone" inputmode="tel" placeholder="Numéro de téléphone (obligatoire)" required><input name="email" type="email" placeholder="E-mail (facultatif)"><input name="password" type="password" minlength="6" placeholder="Mot de passe (6 caractères min.)" required><button class="btn ghost dark-text">Créer mon compte</button></form><p id="authMsg" class="form-msg"></p>`);document.querySelector('#loginForm').onsubmit=e=>authSubmit(e,'/login');document.querySelector('#registerForm').onsubmit=e=>authSubmit(e,'/register');document.querySelector('#forgotPassword').onclick=showForgotPassword}
